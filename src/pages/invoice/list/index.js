...
const InvoiceList = () => {
  ...
  const [startDateRange, setStartDateRange] = useState(new Date('03/01/2023'))
  const [inventoryData, setInventoryData] = useState([])
  const [singleInventoryData, setSingleInventoryData] = useState({})

  const [paginationModel, setPaginationModel] = useState({
    pageSize: 25,
    page: 5
  })
  const [open, setOpen] = useState({ open: false, type: 'view' })
  ...
  const handleClickOpen = async row => {
    ...
    formData.append('invoice_id', row.id)

    const res = await axios.post('https://osa-efris.harissint.com/index.php/api/invoice_details', formData, {})
    setSingleInventoryData(res.data.Result[0])
    setPageState({ ...pageState, isLoading: false })

    setOpen({ open: true, type: 'view' })
  }
  ...
  const columns = [
    {
      ...
      renderCell: params => {
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }} onClick={() => setOpen({ open: true, type: 'number' })}>
  ...
  return (
    <DatePickerWrapper>
      <Grid container spacing={6}>
        ...
        <Dialog fullScreen onClose={handleClose} aria-labelledby='full-screen-dialog-title' open={open.open}>
          ...
          <DialogContent sx={{ '& + .MuiDialogActions-root': { p: theme => `${theme.spacing(3)} !important` } }}>
            {open.type == 'view' ? <Preview singleInventoryData={singleInventoryData} /> : <Email folder='inbox' />}
          </DialogContent>
          {open.type != 'view' ? (
            <DialogActions>
